...
ThreadUsleep(int usecs)
{
	...
	if (pthread_cond_timedwait(&cond, &mutex, &ts) < 0)  {
		perror("pthread_cond_t// Empty Semapore를 waitimedwait");
		pthread_exit(NULL);
	}
...
Producer(void *dummy)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  { 
		if (sem_wait(&EmptySem) < 0)  { // Empty Semaphore wait
			perror("sem_wait");
		...
		if (sem_post(&FullSem) < 0)  { // Full Semaphore signal
			perror("sem_post");
			pthread_exit(NULL);
		}
...
Consumer(void *dummy)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		if (sem_wait(&MutexSem) < 0)  { // Metex semaphore wait
			perror("sem_wait");
			pthread_exit(NULL);
		}
		...
		if (sem_post(&EmptySem) < 0)  { // Empty semaphore signal
			perror("sem_post");
			pthread_exit(NULL);
		}

		ThreadUsleep((rand()%100)*10000); // thread sleep
	}
