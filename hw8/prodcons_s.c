...
void
ThreadUsleep(int usecs) // Thread를 usecs만큼 sleep
{
	pthread_cond_t		cond;
...
Producer(void *dummy)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		ThreadUsleep(data); // Thread random sleep
	}
...
Consumer(void *dummy)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		if (sem_post(&EmptySem) < 0)  { // EMpty semphore siganl
			perror("sem_post");
			pthread_exit(NULL);
		}

		ThreadUsleep((rand()%100)*10000); // thread random sleep
	}
