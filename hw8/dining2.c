...
void
ThreadUsleep(int usecs) //usecs 동안 thread를 sleep하는 함수
{
	pthread_cond_t		cond;
...
Test(int id)
{
	if (State[id] == HUNGRY && State[LEFT(id)] != EATING 
			&& State[RIGHT(id)] != EATING)  { // 양 옆에Eating 중인지 확인
		State[id] = EATING;

		if (sem_post(&Philosopher[id]) < 0)  { // Philosopher[id] semaphore signal
			perror("sem_post");
			pthread_exit(NULL);
		}
	}
...
PickUp(int id)
{
	if (sem_wait(&Mutex) < 0)  { // Mutex semaphore wait
		perror("sem_wait");
		pthread_exit(NULL);
	}
	...
	if (sem_post(&Mutex) < 0)  { // Mutex semaphore signal
		perror("sem_post");
		pthread_exit(NULL);
	}

	if (sem_wait(&Philosopher[id]) < 0)  { // Philosopher[id] signal
		perror("sem_wait");
		pthread_exit(NULL);
	}
}
...
main()
{
	...
	for (i = 0 ; i < NUM_MEN ; i++)  {
		if (sem_init(&Philosopher[i], 0, 0) < 0)  { // Philsopher semaphore 초기화
			perror("sem_init");
			exit(1);
		}
		id[i] = i;
	}
	if (sem_init(&Mutex, 0, 1) < 0)  { // Mutex semaphore 초기화
		perror("sem_init");
		exit(1);
	}
